...
public class RouteController {
    ...
    public ResponseEntity<Route> getRoute(
            ...
            @RequestParam("type") String type) {
        return ResponseEntity.ok(
                routeService.getRoute(start, finish, RouteType.valueOf(type)).get());
    }
    ...
    public ResponseEntity<Route> subscribeForRoute(
            ...
            @RequestParam("time") String time) {
        return ResponseEntity.ok(
                routeService.subscribeToRoute(route, TimeInterval.valueOf(time), account));
    }

    @RequestMapping(
            method = RequestMethod.POST,
            value = "/subscribers-mixed",
            consumes = MediaType.APPLICATION_JSON_UTF8_VALUE,
            produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    public ResponseEntity<List<Account>> getSubscribers(
            @RequestBody @Valid Route route, @RequestParam("time") String time) {
        return ResponseEntity.ok(routeService.getSubcribers(route, TimeInterval.valueOf(time)));
    }
